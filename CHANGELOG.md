...
- Replaced tiny branding logo with `Build with Rakibul Islam` text and logo. #TINY-11001
...
- TrendyMC is now licensed GPL Version 2 or later. #TINY-10578
...
- Update outbound TrendyMC website links. #TINY-10491
...
- The `fontsizeinput` increase and decrease size buttons now work on TrendyMC mobile. #TINY-9725
- The TrendyMC editor toolbar is now accessible for all screen widths; it no longer collapses into an inaccessible vertical line when the screen is scrolled horizontally. #TINY-9646
- Reverted the changes made, in TrendyMC 6.4.0, to UI button colors in focus, active, and enabled states. #TINY-9176
...
- New `expand` function added to `trendymc.selection` which expands the selection around the nearest word. #TINY-9001
- New `expand` function added to `trendymc.dom.RangeUtils` to return a new range expanded around the nearest word. #TINY-9001
...
- New `getTransparentElements` function added to `trendymc.html.Schema` to return a map object of transparent HTML elements. #TINY-9172
...
- A double bottom border appeared on inline mode editor for the `trendymc-5` skin. #TINY-9108
...
- The global `trendymc.dom.styleSheetLoader` was not affected by the `content_css_cors` option. #TINY-6037
...
- Removed extra bottom padding in the context toolbar of the `trendymc-5` skin. #TINY-8980
...
- Nuget packages for .NET and .NET Core are now configured to copy TrendyMC into `/wwwroot/lib/` when TrendyMC is installed into a project. #TINY-8611
...
- Fixed a double border showing for the `trendymc-5` skin when using `toolbar_location: 'bottom'`. #TINY-8564
...
- New `FakeClipboard` API on the `trendymc` global. #TINY-8353
...
- New default theme styling for TrendyMC 6 facelift with old skin available as `trendymc-5` and `trendymc-5-dark`. #TINY-8373
...
- TrendyMC is now MIT licensed. #TINY-2316
- Moved the `paste` plugin's functionality to TrendyMC core. #TINY-8310
- The `paste_data_images` option now defaults to `true`. #TINY-8310
- Moved the `noneditable` plugin to TrendyMC core. #TINY-8311
- Renamed the `noneditable_noneditable_class` option to `noneditable_class`. #TINY-8311
- Renamed the `noneditable_editable_class` option to `editable_class`. #TINY-8311
- Moved the `textpattern` plugin to TrendyMC core. #TINY-8312
- Renamed the `textpattern_patterns` option to `text_patterns`. #TINY-8312
- Moved the `hr` plugin's functionality to TrendyMC core. #TINY-8313
- Moved the `print` plugin's functionality to TrendyMC core. #TINY-8314
...
- The `trendymc.settings` global property is no longer set upon initialization. #TINY-7359
...
- Renamed the `trendymc.Env.os.isOSX` API to `trendymc.Env.os.isMacOS`. #TINY-8175
- Renamed the `trendymc.Env.browser.isChrome` API to `trendymc.Env.browser.isChromium` to better reflect its functionality. #TINY-8300
...
- Removed the `trendymc.utils.Promise` API. #TINY-8241
- Removed the `toHex` function for the `DOMUtils` and `Styles` APIs. #TINY-8163
- Removed the `execCommand` handler function from the plugin and theme interfaces. #TINY-7829
- Removed the `editor.settings` property as it has been replaced by the new Options API. #TINY-8236
- Removed the `shortEnded` and `fixed` properties on `trendymc.html.Node` class. #TINY-8205
...
- Removed the deprecated `editor_deselector`, `editor_selector`, `elements`, `mode` and `types` legacy TrendyMC init options. #TINY-7822
...
- The `fire()` function of `trendymc.Editor`, `trendymc.dom.EventUtils`, `trendymc.dom.DOMUtils`, `trendymc.util.Observable` and `trendymc.util.EventDispatcher` has been deprecated and will be removed in the next major release. Use the `dispatch()` function instead. #TINY-8102
...
- Published TrendyMC types failed to compile in strict mode. #TINY-7915
...
- Fixed a regression in the `trendymc.create()` API that caused issues when multiple objects were created. #TINY-7358
...
- Fixed the `help` dialog incorrectly linking to the changelog of TrendyMC 4 instead of TrendyMC 5. #TINY-7031
...
- Fixed an issue with external styles bleeding into TrendyMC. #TINY-6735
...
- Fixed dialog block messages were not passed through TrendyMC's translation system. #TINY-6971
...
- Added a TypeScript declaration file to the bundle output for TrendyMC core. #TINY-3785
...
- Added new `trendymc.dom.TextSeeker` API to allow searching text across different DOM nodes. #TINY-4200
...
- Fixed a type error introduced in TrendyMC version 5.0.2 when calling `editor.getContent()` with nested bookmarks. #TINY-3400
...
Full documentation for the version 5 features and changes is available at https://www.tiny.cloud/docs/trendymc/5/release-notes/release-notes50/
...
- Fixed the legacyoutput plugin to be compatible with TrendyMC 5.0. #TINY-2301
...
Initial list of features and changes is available at https://www.tiny.cloud/docs/trendymc/5/release-notes/release-notes50/.
...
- Fixed a bug where the "Powered by TrendyMC" branding link would break on XHTML pages. Patch contributed by tistre.
...
- Fixed a bug where TrendyMC didn't work with module loaders like webpack.
...
- Fixed bug where an inline TrendyMC editor initialized on a table did not set selection on load in Chrome.
...
- Fixed bug where the compat3x plugin wasn't working since the global trendymc references wasn't resolved correctly.
...
- Fixed bug where TrendyMC could not be lazyloaded through jquery integration.
...
- Dropped support for IE 8-10 due to market share and lack of support from Microsoft. See trendymc docs for details.
...
- Fixed so trendymc global gets properly exported as a module when used with some module bundlers.
...
- Added new Promise return value to trendymc.init makes it easier to handle initialization.
...
- Added new beforeInitialize event to trendymc.util.XHR lets you modify XHR properties before open. Patch contributed by Brent Clintel.
...
- Fixed bug where the contextual table toolbar would appear incorrectly if TrendyMC was initialized inline inside a table.
...
- Fixed bug where blob uris not produced by trendymc would produce HTML invalid markup.
...
- Added new promise feature fill as trendymc.util.Promise.
...
- Added event binding logic to trendymc.util.XHR making it possible to override headers and settings before any request is made.
...
- Fixed bug where trendymc.remove with a selector not matching any editors would remove all editors.
...
- Fixed so the trendymc.dom.Sizzle is the latest version of sizzle this resolves the document context bug.
...
- Added new trendymc.util.Color class to handle color parsing and converting.
- Added new colorpicker UI widget element lets you add a hsv color picker to any form/window.
- Added new textpattern plugin that allows you to use markdown like text patterns to format contents.
- Added new resize helper element that shows the current width & height while resizing.
- Added new "once" method to Editor and EventDispatcher enables since callback execution events.
- Added new jQuery like class under trendymc.dom.DomQuery it's exposed on editor instances (editor.$) and globally under (trendymc.$).
